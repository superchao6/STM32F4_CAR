...
#define i2c_write   DMP_Write_Len
#define i2c_read    DMP_Read_Len
    ...
    unsigned char *more)
{
    ...
    if (i2c_read(st.hw->addr, st.reg->fifo_r_w, length, data)){printf("6\r\n");
        return -1;}
...
{
	uint8_t res=0;
	MX_I2C3_Init();HAL_Delay(200); 		//初始化IIC总线
